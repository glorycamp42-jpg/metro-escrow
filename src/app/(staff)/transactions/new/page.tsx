...
import { addUserEscrow } from "@/lib/data/userEscrows";
...
export default function NewTransactionPage() {
  ...
  async function submit(ev: React.FormEvent) {
    ...
    const emd = Math.round(price * 0.03);
    const newEscrow: Escrow = {
      ...
      parties: form.buyer ? [{
        ...
        name: form.buyer,
        role: "buyer",
        email: form.buyerEmail || "buyer@example.com"
      }] : [],
      milestones: [],
      risks: [],
      critical: { contractAccepted: new Date().toISOString().slice(0, 10) },
      wire: {
      ...
      settlement: {
        ...
        sellerCredits: 0,
        fees: [],
        buyerCash: Math.round(price * 0.225),
        sellerProceeds: Math.round(price * 0.88)
      },
      portalToken: "tkn_" + form.txnNumber.toLowerCase(),
      step: 1
    };
    addUserEscrow(newEscrow);
    logAudit({
      who: "Jin Yu",
      role: "Officer",
      action: "Escrow opened",
      target: form.txnNumber,
      detail: form.address + " - " + form.buyer + " - $" + price.toLocaleString()
    });
    toast.push("Escrow " + form.txnNumber + " opened and saved", "ok");
    setSubmitting(false);
  ...
  return (
    <div className="max-w-[860px] mx-auto flex flex-col gap-5">
      <div className="flex items-center gap-3">
        <Link href="/transactions" className="text-ink-500 hover:text-ink-800 inline-flex items-center gap-1.5 text-[12px]">
      ...
      <div className="flex items-end justify-between">
        ...
        <Button variant="ink" onClick={open}>
          <Sparkles size={14} /> Open with AI instead
        </Button>
      </div>
        ...
        <form onSubmit={submit} className="flex flex-col gap-5">
          <Section title="Transaction details">
            <Field label="Transaction number" error={errors.txnNumber}>
              <Input value={form.txnNumber} onChange={(e) => set("txnNumber", e.target.value.toUpperCase())} placeholder="TXN-2026-005" invalid={!!errors.txnNumber} />
            </Field>
            <Field label="Type">
          ...
          <Section title="Property">
            <div className="md:col-span-2">
              <Field label="Address" error={errors.address}>
                <Input value={form.address} onChange={(e) => set("address", e.target.value)} placeholder="123 Main St" invalid={!!errors.address} />
              </Field>
            </div>
            <Field label="City" error={errors.city}>
              <Input value={form.city} onChange={(e) => set("city", e.target.value)} placeholder="Los Angeles" invalid={!!errors.city} />
            </Field>
            <Field label="State" error={errors.state}>
              <Input value={form.state} onChange={(e) => set("state", e.target.value.toUpperCase())} placeholder="CA" maxLength={2} invalid={!!errors.state} />
            </Field>
            <Field label="ZIP" error={errors.zip}>
              <Input value={form.zip} onChange={(e) => set("zip", e.target.value)} placeholder="90001" invalid={!!errors.zip} />
            </Field>
          </Section>

          <Section title="Financials">
            <Field label="Purchase price" hint="USD, digits only" error={errors.price}>
              <Input value={form.price} onChange={(e) => set("price", e.target.value)} placeholder="850000" invalid={!!errors.price} />
            </Field>
            <Field label="Closing date" error={errors.closingDate}>
              <Input type="date" value={form.closingDate} onChange={(e) => set("closingDate", e.target.value)} invalid={!!errors.closingDate} />
            </Field>
          </Section>

          <Section title="Buyer">
            <Field label="Full name" error={errors.buyer}>
              <Input value={form.buyer} onChange={(e) => set("buyer", e.target.value)} placeholder="John Buyer" invalid={!!errors.buyer} />
            </Field>
            <Field label="Email (optional)" error={errors.buyerEmail}>
              <Input type="email" value={form.buyerEmail} onChange={(e) => set("buyerEmail", e.target.value)} placeholder="john@example.com" invalid={!!errors.buyerEmail} />
            </Field>
          </Section>
