...
export default function TrustPage() {
  ...
  function exportReconciliation() {
    const rows = escrows
      .map((e) => [e.id, e.property.address, e.settlement.emd, 0, e.settlement.emd, e.wire.callbackVerified ? "verified" : "needs callback"].join(","))
      .join("\n");
    const csv = "File,Property,EMD,Disbursed,Balance,Wire\n" + rows;
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "trust-reconciliation-" + new Date().toISOString().slice(0, 10) + ".csv";
    a.click();
    URL.revokeObjectURL(url);
    logAudit({ who: "Jin Yu", role: "Officer", action: "Trust reconciliation exported", target: "system", detail: escrows.length + " files included" });
    toast.push("Reconciliation exported as CSV", "ok");
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      <header className="flex items-end justify-between">
        <div>
          <h1 className="text-[28px] font-medium tracking-tighter2">Trust accounting</h1>
          <p className="text-[14px] text-ink-500 mt-1">
            Single trust ledger across all escrows. Three-way reconciliation required before any disbursement.
          </p>
        </div>
        <Button variant="secondary" onClick={exportReconciliation}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <KpiCard label="Open files" value={String(open)} sub="with funds in trust" />
        <KpiCard label="Wires verified" value={verifiedWires + " / " + escrows.length} sub={unverifiedWires > 0 ? unverifiedWires + " need callback" : "all verified"} tone={unverifiedWires > 0 ? "warn" : "ok"} />
        <KpiCard label="Last reconciled" value="May 3" sub="3-way matched" tone="ok" />
      </div>
          ...
          <tbody>
            {escrows.map((e) => (
              <tr key={e.id} className="border-t border-cream-200">
                ...
                <td className="py-3 text-right tabular-nums">{fmtMoney(e.settlement.emd)}</td>
...
function KpiCard({ label, value, sub, tone }: { label: string; value: string; sub: string; tone?: "ok" | "warn" }) {
  ...
  return (
    <div className="bg-white border border-cream-300 rounded-md p-4 shadow-card">
      ...
      <p className="text-[12px] mt-1.5" style={{ color }}>{sub}</p>
    </div>
  );
}
